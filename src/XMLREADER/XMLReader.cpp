...
bool XMLReader::readerXML() {
    ...
    if (!doc.LoadFile("C:\\Users\\Dell\\Desktop\\university 2023-2024\\semester 2\\week2\\testFolder\\XMLREADER\\data.xml")) {
    ...
    for (TiXmlElement* deviceElement = root->FirstChildElement("DEVICE"); deviceElement; deviceElement = deviceElement->NextSiblingElement("DEVICE")) {
        ...
        if (deviceInfo.deviceName.empty()) {
            cerr << "Device name is empty for the device." << endl;
            continue;
        }
    ...
    for (TiXmlElement* jobElement = root->FirstChildElement("JOB"); jobElement; jobElement = jobElement->NextSiblingElement("JOB")) {
        ...
        if (!jobNumberText) {
            cerr << "Job number is empty for the device. Continuing onto the next attribute." << endl;
            continue;
        }
        jobInfo.jobNumber = atoi(jobNumberElement->GetText());
        const char* pageCountText = pageCountElement->GetText();
        if (!pageCountText) {
            cerr << "Page count is missing or empty for the device. Continuing onto the next attribute." << endl;
            continue;
        }
        jobInfo.pageCount = atoi(pageCountElement->GetText());
        const char* userNameText = userNameElement->GetText();
        if (!userNameText) {
            cerr << "User name is missing or empty for the device. Continuing onto the next attribute." << endl;
            continue;
        }
